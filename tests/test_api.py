...
class ApiTests(unittest.TestCase):
    def test_submission_returns_id_and_location(self):
        response = Response()
        with patch("api.submit_add", return_value=TASK_ID) as submit:
    ...
    def test_submission_failure_is_503(self):
        with patch("api.submit_add", side_effect=QueueUnavailable("private details")):
    ...
    def test_lookup_failure_is_503(self):
        with patch("api.read_result", side_effect=QueueUnavailable()):
            with self.assertRaises(HTTPException) as caught:
                api.get_demo_task(UUID(TASK_ID))
        self.assertEqual(caught.exception.status_code, 503)

    def test_result_is_returned(self):
        with patch("api.read_result", return_value={"task_id": TASK_ID, "state": "SUCCESS", "result": 5}):
            result = api.get_demo_task(UUID(TASK_ID))
        self.assertEqual(result.result, 5)

    def test_pending_does_not_claim_task_exists(self):
        note = "编号可能不存在或已过期。"
        with patch("api.read_result", return_value={"task_id": TASK_ID, "state": "PENDING", "note": note}):
            result = api.get_demo_task(UUID(TASK_ID))
