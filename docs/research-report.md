...
| 可借鉴点与不可借鉴点 |可借鉴：素材与片段分开建模、区分时间轴位置与素材范围、命令式编辑、自动保存，以及预览和导出共用部分渲染实现。需要另行设计：ClipForge 的服务端任务、跨设备项目恢复和工程交换；不能将浏览器本地持久化直接当作服务器备份，也不能根据单个样本判断长视频处理性能。依据见第 3.1—3.4 节。| 可借鉴：将视频导入、自动转写、模型分析与切片输出串成完整流程，并展示标题、评分和时间区间，供用户预览后下载。不宜直接照搬：当前部署中的前端入口缺失、设置接口模式限制、不适配的后台健康检查和明文密钥日志。系统评分也不能直接作为人工质量评价。依据见第 4.1、4.5 节。 | 待分析 |

## 7. 商业产品体验与交互设计分析

### 7.1 体验目标与范围

本次使用 `Test1.mp4` 体验 OpusClip，重点观察长视频自动生成候选短片、结果评价、人工编辑、项目保存和视频导出的完整流程，为 ClipForge 的需求定义与交互设计提供参考。

本节依据实际操作反馈及界面截图整理。区分已经操作验证的流程、界面中发现的功能，以及尚未开展的质量与性能测试。

本次已完成 OpusClip 的基础体验；Descript 尚未体验，因此不对其实际使用效果作结论。

### 7.2 视频导入与自动生成候选

上传 `Test1.mp4` 后，OpusClip 自动生成多个候选短片。结果页截图展示了 10 个候选，评分从 61 到 99。

每个候选以卡片形式展示，包含视频预览、自动生成的标题、时长、评分，以及编辑、下载等操作入口。用户可以先浏览候选，再选择感兴趣的片段进一步查看和修改。

这种设计将长视频拆解为可逐项检查的候选结果，有助于用户确定优先查看哪些内容。但自动选中的片段是否完整、切点是否自然，仍需结合播放结果判断。
...
体验中观察到，产品从 Hook、Flow、Value、Trend 四个维度对候选片段提供评价。本次将其作为检查片段的提示，关注以下问题：
...
| Trend | 内容是否可能符合当前受众兴趣或传播趋势？ |

以上是本次体验对评价维度的理解，不涉及平台内部评分算法。

评分与评价可以帮助用户筛选候选，但本次没有发布视频或采集传播数据，因此不能将高分等同于真实传播效果，也不能把平台评分作为客观质量结论。

#### 7.3.2 自动生成标题

系统为候选片段自动生成标题。例如，结果页出现了围绕游戏规则、卡牌和角色技能组织的标题。

体验者认为自动标题较有吸引力，能够在播放前帮助理解片段主题。这为用户筛选和后续发布提供了便利，但标题是否准确概括内容、是否存在夸大或偏离原意，仍需人工检查。

**证据：** 候选结果页截图及本次人工体验记录。四维评价来自操作反馈，当前结果页截图主要展示候选总评分和标题。

### 7.4 人工编辑与 AI 辅助体验

#### 7.4.1 文本、预览和时间轴共同展示

编辑器同时展示转写文本、视频预览和时间轴。左侧可以查看转写内容，中间预览画面，下方查看片段排列、视频缩略图和音频波形。

这种布局让用户能够同时检查“说了什么”和“画面如何呈现”，为字幕修正和片段调整提供直观参照。

本次编辑器截图显示，当前片段采用 9:16 竖屏画幅，界面时长显示约为 26 秒。

#### 7.4.2 字幕修改

本次实际修改了自动识别的字幕，并在预览画面中观察到修改后的文字。随后完成保存及重新进入编辑器的检查，用户确认修改结果得到保留。

这说明本次样本中，自动生成的字幕可以继续由用户修正，修改结果能够保存和恢复。

**证据：** 字幕编辑截图及用户对保存恢复检查的确认。

#### 7.4.3 时间轴与编辑功能

体验中观察到视频画面与音频波形分层展示，并发现音量调整、分段、变速、添加效果及 AI 辅助剪辑等功能。

视频和音频的分层呈现有助于理解素材组成，也方便定位需要调整的位置。不过，仅凭界面分层不能推断其内部数据模型，或认定所有音视频操作都支持完全独立编辑。

本次主要完成了基础编辑流程体验，没有对上述每项功能的输出效果逐一进行系统测试。

#### 7.4.4 AI 语音清理

本次实际使用了消除语气词等 AI 辅助功能，体验了在自动生成候选之后继续清理口语内容的操作流程。

这类功能为进一步精简片段提供了入口，但本次没有统计语气词识别准确率、误删比例或节省的编辑时间，也没有开展处理前后的系统对照评价。

**证据：** 用户实际操作反馈；编辑器截图可见 Speech cleanup 入口。

### 7.5 保存、处理反馈与导出

本次完成了修改后的保存恢复及导出验证。相关操作与证据如下：

| 检查项目 | 本次结果与依据 |
| --- | --- |
| 修改字幕 | 已操作；截图中转写文本与视频预览出现修改后的文字。 |
| 保存并重新进入 | 用户确认已完成验证，修改结果保留。 |
| 已编辑状态 | 结果页中的对应候选显示 Edited 标记。 |
| 处理过程反馈 | 候选卡片显示处理中的状态及预计剩余时间。 |
| MP4 下载 | 浏览器下载面板显示对应 MP4 文件下载完成。 |

截图中的“46s left”代表当时界面显示的预计剩余时间，不是本次完整处理耗时。

从交互设计看，已编辑标记帮助用户识别修改过的候选，处理状态和剩余时间提示则帮助用户理解后台任务仍在进行。最终下载入口将编辑结果与成品获取连接起来。

本次未记录最终文件的编码参数、分辨率及精确时长，也未进行逐帧切点和音画同步误差测量。

**证据：** 编辑保存截图、Edited 与处理倒计时截图、浏览器下载完成截图，以及用户操作反馈。

### 7.6 对 ClipForge 的设计启发

| OpusClip 中观察到的设计 | 对 ClipForge 的启发 |
| --- | --- |
| 自动生成多个候选，以卡片展示结果 | 将自动分析结果整理成候选列表，方便逐项预览和选择。 |
| 候选提供评分和多维度评价 | 同时展示评分与选择理由，辅助人工判断，保留用户调整和否决的能力。 |
| 自动生成标题 | 为候选提供便于理解的名称，并允许用户修改。 |
| 文本、视频预览和时间轴共同展示 | 让字幕、片段范围与画面保持可理解的对应关系，降低检查成本。 |
| 自动字幕允许人工修改 | 将字幕识别结果作为可编辑内容，支持修正、保存与导出。 |
| 已编辑标记与处理状态提示 | 明确区分原始候选、人工修改结果、处理中状态和可下载成品。 |
| 保存后可以继续编辑 | 保存人工确认的剪辑安排，避免重复操作。 |
| AI 语音清理 | 可作为后续扩展功能，提供处理前后预览及撤销能力。 |

结合现阶段任务，ClipForge 应优先完成视频导入、后台处理、候选生成、人工复核、字幕修改、项目保存和导出等主流程。

自动标题、语气词清理及更多视觉效果可以作为扩展方向，其优先级应结合开发周期与基础流程完成情况确定。本次体验不意味着需要复制商业产品的全部功能。

### 7.7 体验结论与局限

本次完成了 OpusClip 从视频上传、自动生成候选、查看评价、人工编辑到保存和导出的基础体验，并实际使用了消除语气词等 AI 功能。体验者认为网站整体较好用，自动标题有吸引力，自动生成与人工修改之间衔接较顺畅。

对 ClipForge 最直接的参考，是让系统先生成可解释的候选，再由用户检查和修改，最终形成可保存、可导出的剪辑结果。

本次体验仍存在以下限制：

- 仅使用一个测试素材，不能代表不同语言、内容类型和视频长度下的表现。
- 未量化评估字幕准确率、候选质量、语气词清理效果及处理性能。
- 未验证平台评分与真实观看量、完播率或传播效果的关系。
- 未逐项测试变速、音量调整和各类效果的最终输出。
- 未系统核对不同套餐、免费额度及功能限制。
- Descript 尚未实际体验，后续应补充其文本剪辑流程记录。

本节结论以本次样本和人工操作为范围，不将界面功能入口等同于全部功能验证通过。

### 7.8 证据与参考资料

**实际体验证据：**

1. `Test1.mp4` 候选短片结果页截图：展示候选、标题、时长及评分。
2. 编辑器截图：展示转写文本、竖屏预览、时间轴和音频波形。
3. 字幕修改截图：展示修改后的文字及保存、导出入口。
...
- Descript / Opus Clip 实际体验及跨项目集成验证尚未完成。

各项限制的实验或源码背景见第 3—7 节。

### 8.5 后续工作

1. 补充 Descript / Opus Clip 的实际体验记录；无法体验的功能如实说明原因。
2. 完善需求规格说明书，形成至少 8 条功能需求和 5 条非功能需求，并为每条需求设置验收判据。
3. 绘制 ClipForge 自身的用例图与主流程时序图。
4. 扩充三个参考项目的模块依赖图，并核对六维对比中各项结论的证据。
5. 核对 AutoClip 实际任务执行路径，跟踪已发现问题的处理情况。
6. 对代表性素材补充字幕、切点、音画同步、处理耗时及资源占用检查。
7. 统一归档截图、工程样本、版本和本地修改记录；检查报告引用、排版及页数。
8. 在上述证据基础上完善 ClipForge 的系统设计与选型建议。

当前已完成三个参考项目的基础实验及部分源码调查，第一周交付材料仍需继续完善。
## 9. 参考资料
