...
import clipforge.storage.jobs as store
...
class VideoJobTests(unittest.TestCase):
    ...
    def submit(self, name="sample.mp4", data=b"video placeholder"):
        with patch("clipforge.routes.video.submit_video") as enqueue:
            response = self.client.post("/tasks", files={"file": (name, data, "video/mp4")})
        return response, enqueue

    def test_existing_database_from_before_package_migration_remains_readable(self):
        task_id = str(uuid4())
        ...
        with closing(sqlite3.connect(database)) as connection, connection:
            ...
            connection.execute(
                "INSERT INTO jobs VALUES (?, 'old.mp4', 'source.mp4', 'SUCCEEDED', "
                "'done', 100, '2026-09-22', '2026-09-22', NULL, NULL)", (task_id,),
            )
        result = self.client.get(f"/tasks/{task_id}")
        self.assertEqual(result.status_code, 200)
        self.assertEqual(result.json()["source_name"], "old.mp4")
        self.assertEqual(self.client.get("/tasks").json()["items"][0]["task_id"], task_id)
        self.assertEqual(list(Path(self.temp.name).glob("*.sqlite3")), [database])
    ...
    def test_size_limit_prevents_enqueue(self):
        with patch("clipforge.routes.video.MAX_UPLOAD_BYTES", 4):
    ...
    def test_uncertain_submission_keeps_record_for_late_worker(self):
        with patch("clipforge.routes.video.submit_video", side_effect=QueueUnavailable()):
    ...
    def test_pipeline_failure_is_persisted_without_publishing_download(self):
        response, _ = self.submit()
        task_id = response.json()["task_id"]
        with patch("clipforge.services.pipeline.probe_video", side_effect=ValueError("invalid private path")):
