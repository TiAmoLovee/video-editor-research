...
| 工作项 | 当前状态 |
| --- | --- |
| OpenCut Classic | 已完成本地运行、导入、分割与删除、MP4 导出、播放及刷新恢复；已有部分源码分析和局部模块图 |
| AutoClip | 已完成本地部署、转写、模型分析、切片生成及播放；已有任务、模型调用和数据结构分析，实际任务启动路径仍需运行证据核对 |
| Shotcut / MLT | 已完成手工剪辑、MLT 保存与重新打开、MP4 导出与播放；已有工程读写分析和工程样本 |
...
1. 扩充三个参考项目模块图的覆盖范围，优先补充 AutoClip 的分析与输出关系，以及 Shotcut 的播放、滤镜和导出关系。
